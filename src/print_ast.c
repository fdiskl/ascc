...
static void print_stmt(stmt *s, int indent) {
  ...
  case STMT_LABEL:
    ...
    return;
  case STMT_WHILE:
    if (s->v.while_stmt.break_label_idx == -1 ||
    ...
    return;
  case STMT_DOWHILE:
    if (s->v.dowhile_stmt.break_label_idx == -1 ||
    ...
    return;
  case STMT_FOR:
    if (s->v.for_stmt.break_label_idx == -1 ||
  ...
  case STMT_CASE:
    ...
    return;
  case STMT_DEFAULT:
    if (s->v.default_stmt.label_idx == -1)
      printf("DefaultStmt");
    else
    ...
    return;
  case STMT_SWITCH:
    if (s->v.switch_stmt.break_label_idx == -1)
      printf("SwitchStmt");
    else if (s->v.switch_stmt.default_stmt != NULL)
